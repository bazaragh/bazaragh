...
def test_offer_add(client, mocker):
    with client.session_transaction() as session:
        response = client.post("login", data={"email": "admin@bazaragh.pl", "password": "test"})
    with client:
        mock_change_offer_images_filenames = mocker.patch("app.views.offer.change_offer_images_filenames")
        mock_create_offer_images_dir = mocker.patch("app.views.offer.create_offer_images_dir")
        mock_save_offer_images = mocker.patch("app.views.offer.save_offer_images")
        ...
        offer = dict(author=1,
                      ...
                      is_used=True,
                      images=[])
        response = client.post("/user/offer/add", data=offer, follow_redirects=True)
...
def test_offer_edit(client, mocker):
    ...
    with client:
        ...
        mock_create_offer_images_dir = mocker.patch("app.views.offer.create_offer_images_dir")
        mock_save_offer_images = mocker.patch("app.views.offer.save_offer_images")
        mock_delete_offers_images = mocker.patch("app.views.offer.delete_offers_images")
        ...
        response = client.post(f"/user/offer/edit/{offer_id}", data=offer, follow_redirects=True)
        assert response.status_code == 200
        assert response.request.path == url_for('bp_user.offers_get')
        ...
        edited_offer = db.session.query(Offer).order_by(Offer.id.desc()).first()
...
def test_offer_delete(client, mocker):
    ...
    with client:
        ...
        response = client.post(f"/user/offer/delete/{offer_id}", follow_redirects=True)
        assert response.status_code == 200
        assert response.request.path == url_for('bp_user.offers_get')

        deleted_offer = db.session.query(Offer).filter_by(id=offer_id).one_or_none()
...
def test_offer_add_favourite_is_author(client, mocker):
    ...
    with client:
        ...
        response = client.post(f"/user/offer/add-favourite/{offer_id}", follow_redirects=True)
        assert response.status_code == 200
        assert response.request.path == url_for('bp_main.offer_get', offer_id=offer_id)

        favourite = db.session.query(Favourite).filter_by(offer=offer_id, user=1).one_or_none()
