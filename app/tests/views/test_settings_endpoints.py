...
def test_change_email_email_taken(client):
    ...
    with client:
        ...
        response = client.post('/user/settings/change_email', data={"email": new_email}, follow_redirects=True)
...
def test_delete_account(client):
    ...
    with client:
        ...
        response = client.post('/user/settings/delete', data={"email": email}, follow_redirects=True)
...
def test_set_profile_picture(client, mocker):
    with client.session_transaction() as session:
        response = client.post("login", data={"email": "admin@bazaragh.pl", "password": "test"})
    with client:
        mock_change_user_profile_picture_filename = mocker.patch("app.views.settings.change_user_profile_picture_filename")
        mock_delete_user_profile_picture = mocker.patch("app.views.settings.delete_user_profile_picture")
        mock_save_user_profile_picture = mocker.patch("app.views.settings.save_user_profile_picture")
