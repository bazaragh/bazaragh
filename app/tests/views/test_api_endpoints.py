...
def test_api_message(client, mocker):
    ...
    with client:
        ...
        message = db.session.query(Message).order_by(Message.post_date.desc()).first()

        assert message is not None
        assert message.content == content
        mock_socketio_emit.asssert_called_once_with(recipient, {"from": 1, "content": content})
...
def test_api_unread_messages(client, mocker):
    ...
    with client.session_transaction() as session:
        response = client.post("login", data={"email": "admin@bazaragh.pl", "password": "test"})
        response = client.get("/api/messages/unread", content_type='application/json')
        previous_unread = json.loads(response.data)['unread']

        response = client.post("login", data={"email": "test1@bazaragh.pl", "password": "test"})
    ...
    with client:
        curent_unread = client.get("/api/messages/unread", content_type='application/json').json['unread']
...
def test_set_user_offer_rating(client, mocker):
    ...
    with client:
        ...
        rating = 2
        data = {'rating': rating}
        response = client.post(f"/api/offer/{offer_id}/rating", data=json.dumps(data), content_type='application/json')
        ...
        offer_score = db.session.query(OfferScore).filter_by(offer=offer_id, customer=customer_id).one_or_none()
...
def test_set_user_author_rating(client, mocker):
    ...
    with client:
        ...
        rating = 2
        data = {'rating': rating}
        response = client.post(f"/api/offer/{author_id}/author-rating", data=json.dumps(data), content_type='application/json')
        ...
        user_score = db.session.query(UserScore).filter_by(seller=author_id, customer=customer_id).one_or_none()
